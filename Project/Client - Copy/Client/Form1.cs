...
namespace Client
{
    public partial class Form1 : Form
    {
                    ...
                    switch (tokens[0])
                    {
                        ...
                        case "START":
                            ...
                            if (playerType == "Regular")
                            {
                                ...
                                Controls.Find("buttonAutobusComplete", true)[0].Enabled = true;
                            }
                        ...
                        case "FINISH":
                            ...
                            if (playerType == "Regular")
                            {
                                ...
                                Controls.Find("buttonAutobusComplete", true)[0].Enabled = false;
                        ...
                        case "ALL":
                            if (playerType == "Referee")
                            {
                                ...
                                Controls.Find("buttonSendGrades", true)[0].Enabled = true;
                            }

                            break;
                        case "RESULT":
                            ResultsDialog Result = new ResultsDialog(tokens[1]);
                            Result.ShowDialog();
                            break;
        ...
        private void ButtonOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (playerType == "Referee" && Connected)
                {
                    ...
                    {
                        playerCount = int.Parse(TextBoxNoOfPlayers.Text);
                    ...
                    else
                    {
                        ...
                        if (ComboBoxAvailableRooms.SelectedIndex >= 0)
                        {
                            ...
                            playerCount = int.Parse(SplitRoomString[2]);
                            CreateRefereeUI(playerCount);
                        }
                    }
                }
        ...
        private void buttonSendGrades_Click(object sender, EventArgs e)
        {
            //Invoked only through the referee
            ...
            for (int i = 0; i < playerCount; i++)
            {
                for (int u = 1; u <= 5; u++)
                {
                    TotalGrade += (int)((NumericUpDown)Controls.Find("numericUpDown_" + u.ToString() + "_" + i.ToString(), true)[0]).Value;
                }
                TotalGrades += TotalGrade.ToString() + ";";
                TotalGrade = 0;
            }

            string[] GradesTokens = TotalGrades.Split(';');
            ...
            for (int i = 0; i < playerCount; i++)
            {
                for (int u = 1; u <= 6; u++)
                {
                    if (u < 6)
                    { 
                        Grades += ((TextBox)Controls.Find("textBox_" + u.ToString() + "_" + i.ToString(), true)[0]).Text;
                        Grades += " (" + ((NumericUpDown)Controls.Find("numericUpDown_" + u.ToString() + "_" + i.ToString(), true)[0]).Value.ToString() + ")\t";
                    }
                    if (u == 6)
                    {
                        Grades += "\tID: " + ((TextBox)Controls.Find("textBox_" + u.ToString() + "_" + i.ToString(), true)[0]).Text;
                        Grades += "\tTotal: " + GradesTokens[i] + "\n\n";
                    }
                }
            }
            bw.Write(Grades);

            Controls.Find("buttonSendGrades", true)[0].Enabled = false;
        }
        ...
        private void CreatePlayerUI()
        {
            ...
            buttonAutobusComplete.Enabled = false;
        ...
        private void CreateRefereeUI(int playerCount)
        {
            ...
            Button buttonSendGrades = new Button();
            // 
            // buttonSendGrades
            // 
            buttonSendGrades.Location = new System.Drawing.Point(436, 71 + 61 * i);
            buttonSendGrades.Name = "buttonSendGrades";
            buttonSendGrades.Size = new System.Drawing.Size(136, 23);
            buttonSendGrades.TabIndex = 11;
            buttonSendGrades.Text = "Send grades";
            buttonSendGrades.UseVisualStyleBackColor = true;
            buttonSendGrades.Enabled = false;
            buttonSendGrades.Click += new System.EventHandler(this.buttonSendGrades_Click);

            Controls.Add(buttonSendGrades);

            // if the total height is less than the screen height
            if (i * 61 + 150 < 700)
                this.Height = i * 61 + 170;
            else
                this.Height = 700;
        }
        //
        ...
        private void HandleAllforRef(string[] tokens)
        {
            ...
            for (int i = 0; i < playerCount; i++)
            {
                for (int u = 1; u <= 6; u++)
                {
                    AnsTextBox = (TextBox)Controls.Find("textBox_" + u.ToString() + "_" + i.ToString(), true)[0];
                    AnsTextBox.Text = tokens[tokensIndex];
                    tokensIndex++;
                }
            }
        }
